...
# Function to verify label distribution
def verify_label_distribution(y, set_name):
    label_counts = np.sum(y, axis=0)
    total_instances = y.shape[0]
    label_percentages = label_counts / total_instances * 100
    print(f"Label distribution in {set_name}:")
...
def process_dataset(df, identifier):
    ...
    def objective(trial, csv_file=csv_file):
        try:
            ...
            label_counts_fold = np.sum(y_train_fold, axis=0)
            class_weights = 1.0 / (label_counts_fold + 1e-6)  # Avoid division by zero
    ...
    study.optimize(objective_with_csv, n_trials=10000)  # Adjust number of trials as needed
    ...
    report = classification_report(y_true, y_pred, target_names=[f'Class {i}' for i in range(y_true.shape[1])], zero_division=0)
    print(f"Classification Report for {identifier}:\n{report}")

    # Compute Hamming Loss
    hl = hamming_loss(y_true, y_pred)
    print(f"Hamming Loss for {identifier}: {hl}")
